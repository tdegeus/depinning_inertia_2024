...
def Paraview(cli_args=None):
    """
    Convert :py:func:`depinning_inertia_2024.EventMap.Run` output to be viewed in Paraview.
    """
...
def _Info_parser():
    parser = argparse.ArgumentParser(
        formatter_class=QuasiStatic.MyFmt, description=textwrap.dedent(Info.__doc__)
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=str,
        help="Files to read (generated by :py:func:`depinning_inertia_2024.EventMap.Run`)",
    )
    return parser
...
def Info(cli_args=None):
    """
    Collect basic information from :py:func:`depinning_inertia_2024.EventMap.Run`.
    Combine in a single output file.
    """
    parser = _Info_parser()
    args = tools._parse(parser, cli_args)
