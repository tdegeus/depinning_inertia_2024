...
def _UpdateData_parser():
    parser = argparse.ArgumentParser(
        formatter_class=QuasiStatic.MyFmt, description=UpdateData.__doc__
    )
...
def _FilterCompleted_parser():
    parser = argparse.ArgumentParser(
        formatter_class=QuasiStatic.MyFmt, description=FilterCompleted.__doc__
    )
...
def Run(cli_args=None):
    """
    Trigger all branches stored in a file generated by :py:func:`Generate`.
...
def _Generate_parser():
    parser = argparse.ArgumentParser(
        formatter_class=QuasiStatic.MyFmt, description=Generate.__doc__
    )
...
def Generate(cli_args=None):
    ...
    Per realisation in the output of :py:func:`QuasiStatic.EnsembleInfo` create one file
...
def _Merge_parser():
    parser = argparse.ArgumentParser(formatter_class=QuasiStatic.MyFmt, description=Merge.__doc__)
    parser.add_argument("-v", "--version", action="version", version=version)
...
def _MergeBatch_parser():
    parser = argparse.ArgumentParser(
        formatter_class=QuasiStatic.MyFmt, description=MergeBatch.__doc__
    )
    parser.add_argument("-v", "--version", action="version", version=version)
...
def MergeBatch(cli_args=None):
    """
    Run :py:func:`Merge` on a batch of files.
    The usage is::

        :py:func:`MergeBatch` -o delta-f=x /path/to/old/delta-f=x/id*h5
    """
...
def _JobRerun_parser():
    parser = argparse.ArgumentParser(
        formatter_class=QuasiStatic.MyFmt, description=JobRerun.__doc__
    )
