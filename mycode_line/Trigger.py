...
def cli_run(cli_args=None):
    ...
    An option is available to rerun a previous push, just to check.
    """
    ...
    with h5py.File(args.file, "a" if args.check is None else "r") as file:
        ...
        branches = np.arange(file["/branch/step"].size)

        if args.check is not None:
            assert np.all(np.in1d(args.check, branches))
        ...
        for ibranch in pbar:
            ...
            group = file[f"/branch/items/{ibranch:d}"]

            if group["p"][1] > 0 or args.check is not None:
                try_p = np.array([group["p"][1]])
                assert np.all(np.logical_and(try_p >= 0, try_p < system.N))
            else:
                try_p = group["try_p"][1] + np.arange(system.N)
                try_p = np.where(try_p < system.N, try_p, try_p - system.N)

            for p in try_p:

                system.restore_quasistatic_step(group, 0)
                inc = system.inc
            ...
            if args.check is not None:
                ...
                assert group["A"][1] == A
                assert group["T"][1] == T
                assert group["inc"][1] == system.inc
                assert np.isclose(group["f_frame"][1], f_frame)
                assert np.allclose(group["x"]["1"][...], system.x)

            else:

                group["x"]["1"] = system.x
                storage.dset_extend1d(group, "inc", 1, system.inc)
                storage.dset_extend1d(group, "x_frame", 1, system.x_frame)
                storage.dset_extend1d(group, "completed", 1, True)
                storage.dset_extend1d(group, "p", 1, p)
                storage.dset_extend1d(group, "S", 1, S)
                storage.dset_extend1d(group, "A", 1, A)
                storage.dset_extend1d(group, "T", 1, T)
                storage.dset_extend1d(group, "f_frame", 1, f_frame)

                # prepare the next push
                storage.dset_extend1d(group, "p", 2, -1)
                storage.dset_extend1d(group, "try_p", 2, group["try_p"][1])
    ...
    with h5py.File(args.output, "w") as output:
        ...
        for i, (filename, filepath) in enumerate(zip(pbar, args.files)):
            ...
            with h5py.File(filepath) as file:
                ...
                for ibranch in np.arange(file["/branch/step"].size):

                    group = file[f"/branch/items/{ibranch:d}"]

                    if not group["completed"][1]:
                        continue
                    ...
                    T.append(group["T"][1])
                    p.append(group["p"][1])
                    f_frame.append(group["f_frame"][1])
                    f_frame_0.append(group["f_frame"][0])
                    step.append(file["/branch/step"][ibranch])
                    step_c.append(file["/branch/step_c"][ibranch])
...
def cli_generate(cli_args=None):
    ...
    (``n`` in total)::
        ...
        /branch/step        # per branch, the last quasistatic step it was based on [n]
        /branch/step_c      # per branch, the last system-spanning step it was based on [n]
        /branch/loaded      # per branch, true is elastic loading was applied to ``step`` [n]

        # I.    Definition of the state

        /branch/items/{ibranch:d}/x/0         # particle positions [N]
        /branch/items/{ibranch:d}/x_frame     # frame position per step (*)
        /branch/items/{ibranch:d}/inc         # increment per step (*)

        # III.  Trigger control parameters

        /branch/items/{ibranch:d}/try_p       # particle to try to trigger first (**)
        /branch/items/{ibranch:d}/p           # triggered particle (**)

        # IV.   Basic output that cannot be reconstructed

        /branch/items/{ibranch:d}/completed   # check that the dynamics finished (*)
        /branch/items/{ibranch:d}/T           # duration of the event (*)

        # V.    Basic output that can be reconstructed from "x"/"x_frame"

        /branch/items/{ibranch:d}/S           # number of times that blocks yielded (***)
        /branch/items/{ibranch:d}/A           # number of blocks that yielded (***)
        /branch/items/{ibranch:d}/f_frame     # frame force (***)
    """
    ...
    with h5py.File(args.ensembleinfo) as info:
        ...
        for filename in tqdm.tqdm(files):
            ...
            with h5py.File(os.path.join(basedir, filename)) as source, h5py.File(
                ...
                meta = dest.create_group("/branch/items")
